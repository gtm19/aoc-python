...
def get_invalid_ids(id_range: str, repeated: bool = False) -> list[int]:
    ...
    for i in range(start, stop + 1):
        regex = r"^(\d+)\1" + ("$" if not repeated else "+$")
...
def part_one(data: list[str]) -> int:
    ...
    invalid_ids = [id for d in data for id in get_invalid_ids(d)]
    return sum(invalid_ids)


def part_two(data: list[str]) -> int:
    """Solve part two of the day's challenge."""
    invalid_ids = [id for d in data for id in get_invalid_ids(d, repeated=True)]
    return sum(invalid_ids)
